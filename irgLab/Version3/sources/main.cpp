...
int main(int argc, char* argv[]) {
	...
	FPSManager FPSManagerObject(window, 120, 1.0, "Zadatak 6");
	...
	while (glfwWindowShouldClose(window) == false) {
		...
		//renderer.render(*sjencar[1]);
		for (unsigned int i = 0; i < renderer.objects.size(); i++) {
			glUseProgram(sjencar[i]->ID);
			glUniform3f(eyeUniformLocation, camera.camPosition.x, camera.camPosition.y, camera.camPosition.z);
			renderer.objects[i].drawObject(*sjencar[i]);
		}
